...
To start, I used SNP data from GWAS studies to begin finding similarities between asthma and other autoimmune diseases specifically. For each dataset of diseases, I needed to extract the needed information. In this case, it was the allele difference and the gene that the SNP came from. Afterward, I could make a venn diagram to visualize the SNP locations that were the same in different diseases.

```{r pressure, echo = FALSE, fig.cap= "Compare IBD, Graves, Celiac, Asthma", out.width='100%'}
knitr::include_graphics("https://github.com/ElliePerkins-PetitBlanc/senior_project/blob/main/SNPs_venn_diagramm2.png")
```
